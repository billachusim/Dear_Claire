      ...
      if (user != null) {
        ...
        if (!doc.exists) {
          ...
          final String randomAvatarUrl =
              clairevatarUrls[Random().nextInt(clairevatarUrls.length)];

          final userData = {
            "isPremium": false,
            "nickname": user.displayName ?? "Claire User",
            ...
            "email": user.email,
            "fcmId": fcmToken ?? '',
            "secretCode": "", // No secret code for Google login
    ...
    try {
      ...
      if (user != null) {
        ...
        if (!doc.exists) {
          ...
          final userData = {
            "isPremium": false,
            "nickname": appleCredential.givenName ?? user.displayName ?? "Claire User",
            "avatarUrl": user.photoURL ?? randomAvatarUrl,
            "userId": user.uid,
            "languagePreference": "en", // Default
            "alterEgoAccessCode": "",
            ...
            "email": user.email,
            "fcmId": fcmToken ?? '',
            "secretCode": "", // No secret code for Apple login
            "timeLastUnlocked": FieldValue.serverTimestamp(),
            "timeRegistered": FieldValue.serverTimestamp(),
            "userType": "REGULAR",
            "sessionCount": 0,
      ...
      showToast(message: "Apple Sign-In failed.");
      return false;
    }
  }
